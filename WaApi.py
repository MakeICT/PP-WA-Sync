...
class WaApiClient(object):
    ...
    def authenticate_with_apikey(self, api_key, scope=None):
        ...
        #Printout=True
        Printout=False
        self._token = WaApiClient._parse_response(response, Printout)
    ...
    def execute_request(self, api_url, api_request_object=None, method=None, Printout=False, Test=False):
        """
        ...
        if (Printout == True):
                print(request.data)
        if (Test == True):
                print(repr(request.full_url))
        if (Test == False):
                try:
                        response = urllib.request.urlopen(request)
                        return WaApiClient._parse_response(response, Printout)
    ...
    def _refresh_auth_token(self):
        print("Refreshing Auth Token, WaApi", file=sys.stderr)
        data = {
    ...
    @staticmethod
    def _parse_response(http_response, Printout=False):
