...
def CheckInvoices(contact_id, value, transaction_date):
    ...
    for invoice in invoices.Invoices:
        ...
        if invoice.Value == float(value):
            ...
            if abs((transaction_date.date() - invoice_date.date()).days) <= 15:
                ...
                if(db.session.query(db.Payment).get(transaction.transaction_id)):
                    ...
                    if res:
                        ...
                        if not test_mode and (update or auto_update):
                            if not renewal_date or wa_user.MembershipLevel.Name == "Non-Member":
                                input("No active membership! Manual intervention required\n"
                                      "Fix membership status and re-run.")
                            elif (abs((renewal_date - new_renewal_date).days) > 3):
                                ...
                                print("!!Not creating invoice, membership is already current!!")
                    ...
                    # else:                        
                        ...
                        print("Creating payment without an invoice. This is likely due to a double payment. Please double check this transaction.")
                    ...
                    if not test_mode and (update or auto_update):
                        ...
                        if invoice_to_pay:
                            ...
                            print(f"Created WA payment {wa_payment.Id} from PP transaction {transaction.transaction_id}")
